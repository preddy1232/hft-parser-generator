...
docker run --rm -v "${PWD}:/app" hft-parser-env /bin/bash -c "cmake -B build -DCMAKE_BUILD_TYPE=Release -DENABLE_NATIVE_ARCH=ON && cmake --build build -j`$(nproc) && ctest --test-dir build --output-on-failure && ./build/itch_bench && ./build/itch_lob_bench"
