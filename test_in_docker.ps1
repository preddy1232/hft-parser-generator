...
docker run --rm -v "${PWD}:/app" hft-parser-env /bin/bash -c "cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j`$(nproc) && ./build/itch_tests && ./build/itch_driver"
