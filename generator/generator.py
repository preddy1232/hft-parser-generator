...
def emit_accessors(field: dict) -> list[str]:
    ...
    # ── Alpha array (fixed-length string) ────────────────────────────────
        ...
        lines.append(
            ...
            f'{{ return detail::trim_right({name}()); }}'
        )
    ...
    # ── 48-bit timestamp (uint8_t[6] → uint64_t) ────────────────────────
        ...
        lines.append(
            ...
            f'{{ return detail::read_be48(reinterpret_cast<const char*>({name}_)); }}'
        )
    ...
    # ── Standard integers (16/32/64) ─────────────────────────────────────
        ...
        lines.append(
            ...
            f'{{ return detail::{reader}(reinterpret_cast<const char*>(&{name}_)); }}'
        )
...
def generate(schema_path: str, output_path: str) -> None:
    ...
    # ─── File Header ─────────────────────────────────────────────────────
    out.append("// " + "=" * 77)
    out.append("// generated_itch_parser.hpp")
    out.append("//")
    ...
    out.append("//")
    out.append("// Zero-copy, zero-allocation NASDAQ ITCH 5.0 parser.")
    ...
    # ─── Stream Parser ───────────────────────────────────────────────────
    out.append("// " + "─" * 77)
    out.append("// MoldUDP64 / PCAP Stream Parser")
    out.append("//")
    out.append("// Loops through a contiguous buffer of length-prefixed ITCH messages.")
    out.append("// In ITCH / MoldUDP64, each message is often prefixed by a 2-byte length.")
    out.append("// Returns the number of bytes processed.")
    out.append("// " + "─" * 77)
    out.append("template<typename Handler>")
    ...
    out.append("    while (offset + 2 <= length) {")
    out.append("        // Read 2-byte message length (big-endian)")
    out.append("        uint16_t msg_len = detail::read_be16(buffer + offset);")
    out.append("        if (offset + 2 + msg_len > length) break; // Incomplete message")
    out.append("")
